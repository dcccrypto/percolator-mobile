...
def radial_glow(img: Image.Image, cx: int, cy: int, radius: int,
    ...
    for i in range(steps, 0, -1):
        ...
        draw.ellipse([cx - r_i, cy - r_i, cx + r_i, cy + r_i], fill=(*color, a_i))
...
def make_italic_glyph(char: str, font_size: int, fg_color: tuple,
    ...
    # Auto-crop to non-transparent bounding box (removes all excess padding)
    content_box = layer.getbbox()
    if content_box:
        # Add pad back around the tight crop
        l, t, r, b = content_box
        l = max(0, l - pad)
        t = max(0, t - pad)
...
def place_glyph(canvas: Image.Image, glyph: Image.Image, cx: int, cy: int) -> Image.Image:
    """Paste `glyph` (RGBA) centred at (cx, cy) onto `canvas` (RGBA), return composite."""
    gw, gh = glyph.size
    x = cx - gw // 2
    y = cy - gh // 2
...
def generate_app_icon():
    ...
    # Background glow
    img = radial_glow(img, cx, cy, 360, ACCENT, 0.28)
    ...
    # Glyph glow (blurred purple P behind white P)
    font_size = 480
    ...
    img = place_glyph(img, glyph_glow_blur, cx, cy)
...
def generate_splash():
    ...
    img = radial_glow(img, cx, cy, 420, ACCENT, 0.32)
    img = radial_glow(img, cx, cy, 640, ACCENT, 0.12)
